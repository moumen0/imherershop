...
$(document).on("ready", function() {
	...
	if($(".lx-total-costs strong").length){
		...
		$(".lx-total-costs strong").text(price.toFixed(0) + "DA");
...
$(".lx-product-qty .lx-plus").on("click",function(){
	...
	if($("#qty").length){
		...
		$(this).parent().parent().parent().find(".lx-price-total strong").text(($(this).attr("data-price") * $(this).prev("input").val()).toFixed(0) +  "DA");
		var price = 0;
...
$(".lx-product-qty .lx-minus").on("click",function(){
	...
	if($("#qty").length){
		...
		$(this).parent().parent().parent().find(".lx-price-total strong").text(($(this).attr("data-price") * $(this).next("input").val()).toFixed(0) +  "DA");
		var price = 0;
		$(".lx-cart-products-list .lx-price-total strong").each(function(){
			price += parseFloat($(this).text());
		})
	...
	$.ajax({
		...
		success : function(response){
			if(response == 0)
			{
				...
				var tp = parseInt($(".totalprice").text().replace("DA", ""));

				var ship = parseInt($(".ship").text().replace("DA", ""));


				var disc = (tp - ship) * parseInt(response) / 100;

				$(".couponUse").html("الكوبون <b>- "+ disc +" DA</b>");
				...
				$(".lx-total-costs strong").html(totalprice + " DA");
...
$(".lx-cart-info-address select[name='city']").on("change",function(){
	if($(".lx-shipping-costs b").text() !== "0DA"){
		if($(this).val() === "Casablanca"){
			$(".lx-shipping-costs b").text("25DA");
		}
		else{
			$(".lx-shipping-costs b").text("45DA");
		}		
	}
	var price = 0;
	$(".lx-cart-products-list .lx-price-total strong").each(function(){
